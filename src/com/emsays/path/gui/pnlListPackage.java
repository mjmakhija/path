...
public class pnlListPackage extends javax.swing.JPanel implements MyPanelBack
{

	MyFormWrapper vParent;
	List<PackageDTO> vLstObjPackage;
	String[] vColumnNames =
	{
		"#",
		"Id",
	 ...
	 */
	public pnlListPackage(MyFormWrapper vParent)
	{
		initComponents();
	...
	private void mLoadData(List<PackageDTO> vLstObjPackage)
	{
		this.vLstObjPackage = vLstObjPackage;

		DefaultTableModel vDTM = new DefaultTableModel();
		vDTM.setColumnIdentifiers(vColumnNames);
		int i = 1;
		for (PackageDTO vObjPackage : vLstObjPackage)
		{
			Object[] o = new Object[]
			{
				i++,
	...
	{
		this.mLoadData(new PackageSer(Database.getSessionCompanyYear()).retrieve(txtName.getText()));
	}
	...
	private void handleClickBtnEdit()
	{
		...
		this.vParent.addPanel(new pnlAddPackage(vParent,this, vLstObjPackage.get(tbl.getSelectedRow())));
	...
	private void handleClickBtnDelete()
	{
		...
		else
		{
			...
			{
				PackageSer objPackageSer = new PackageSer(Database.getSessionCompanyYear());

				int[] vSelectedIndices = tbl.getSelectedRows();
				for (int i : vSelectedIndices)
				{
					objPackageSer.delete(vLstObjPackage.get(i));
				}
				//mLoadData();
